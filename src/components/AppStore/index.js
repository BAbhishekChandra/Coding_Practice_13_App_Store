...
class AppStore extends Component {
  state = {activeTabItem: tabsList[0].tabId, searchInputValue: ''}
  ...
  render() {
    const {activeTabItem, searchInputValue} = this.state
    // console.log(activeTabItem)
    const filteredAppsList = appsList.filter(
      eachApp =>
        eachApp.category.toLowerCase() === activeTabItem.toLowerCase() &&
